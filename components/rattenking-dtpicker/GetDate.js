const GetDate = {
  ...
  getLoopArray: (start, end,type) => {
    ...
    for (var i = start; i <= end; i++) {
			// if(type){
			// 	if(type == 'hours'){
			// 		array.push(GetDate.withData(i)+' 时');
			// 	}else if(type == 'minutes'){
			// 		array.push(GetDate.withData(i)+' 分');
			// 	}else if(type == 'seconds'){
			// 		array.push(GetDate.withData(i)+' 秒');
			// 	}
			// }else{
  ...
  getDateTimes: (opts) => {
    ...
    var hours = GetDate.getLoopArray(0, 23,'hours');
    var minutes = GetDate.getLoopArray(0, 59,'minutes');
    var seconds = GetDate.getLoopArray(0, 59,'seconds');
    var times = null;
		// console.log(minutes)
    switch (opts.fields) {
      case 'year':
