	...
	export default {
		...
		created() {
			if(this.val === ''){
				...
				this.val = GetDate.format(arr);
				this.curValue = GetDate.format(arr);
			}
			switch (this.fields){
				case 'year':
				  if (this.val.length !== 4) {GetDate.error('时间粒度和时间格式不一致');this.curDisabled = true;return false;}
				  ...
				  break;
				case 'month':
				  if (this.val.length !== 7) { GetDate.error('时间粒度和时间格式不一致'); this.curDisabled = true; return false;}
				  ...
				  break;
				case 'day':
				  if (this.val.length !== 10) { GetDate.error('时间粒度和时间格式不一致'); this.curDisabled = true; return false;}
				  ...
				  break;
				case 'hour':
				  if (this.val.length !== 13) { GetDate.error('时间粒度和时间格式不一致'); this.curDisabled = true; return false;}
				  ...
				  break;
				case 'minute':
				  if (this.val.length !== 16) { GetDate.error('时间粒度和时间格式不一致'); this.curDisabled = true; return false;}
				  ...
				  break;
				case 'second':
				  if (this.val.length !== 19) { GetDate.error('时间粒度和时间格式不一致'); this.curDisabled = true; return false;}
			  ...
			  let targets = GetDate.getCurrentStringValue(this.val);

			  if (GetDate.getTimes(this.val) < GetDate.getTimes(this.start)){
				GetDate.error('默认时间不能小于开始时间')
				this.curDisabled = true
				return false;
			  }
			  if (GetDate.getTimes(this.val) > GetDate.getTimes(this.end)) {
			  ...
			  let times = GetDate.getDateTimes({
					...
					curyear: this.val.substring(0, 4),
					curmonth: this.val.substring(5, 7),
					fields: this.fields
			  })
