		...
		<view class="refresh-moudle"
		  ...
		  <view class="pull-refresh">
				<slot name="pull-refresh">
					<view class="down-tip" v-if="dropDownState!==3">
						<uni-icons class="downicon" :class="{rotate:dropDownState == 2}" type="arrowthindown" size="24"></uni-icons>
						<view class="down-text">{{dropDownInfo.downText}}</view>
					</view>
					<view class="refresh-tip" v-if="dropDownState==3">
						<!-- <uni-icons class="spinner" v-if="dropDownInfo.refreshImg" type="reload" size="24"></uni-icons> -->
						<image class="loaddingImg" v-if="dropDownInfo.refreshImg" src="../../static/loading.gif" mode=""></image>
		...
		data () {
			return {
			  ...
			  dropDownInfo: {
					...
					refreshText: '正在刷新数据...',
					refreshImg: '/static/ygc-refresh/loading.gif'
			  },
			  scrollTop: 0,
					...
					if (e.changedTouches[0].pageY > this.startY) {
						...
						if (!this.isRefreshing) {
						  ...
						  if (this.top >= this.defaultOffset) {
								...
								this.dropDownInfo.downText = '松开立即刷新'
								e.preventDefault();
						  } else { 
							// console.log('dropDownState = 1');
							this.dropDownState = 1;
							this.dropDownInfo.downText = '下拉可以刷新'
							// 去掉会导致ios无法刷新
							e.preventDefault();
						  }
						}
